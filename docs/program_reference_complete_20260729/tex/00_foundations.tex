...
\hypersetup{
  ...
  linkcolor=blue,
  urlcolor=blue,
  pdftitle={ソーラーカーMPCプログラムを読むための基礎知識},
  pdfauthor={Codex}
}
\lstset{
...
\title{ソーラーカーMPCプログラムを読むための基礎知識}
                ...
                \section{プログラム、プロセス、メモリ、オブジェクトを区別する}
...
\section{class、独自クラス、インスタンス、self、継承}
...
\section{先頭アンダースコア、\_\_init\_\_、名前付けの作法}
...
\section{module、package、import、console\_scripts}
...
\section{例外、try/finally、with、資源解放}
...
\section{list、dict、deque、NumPy配列、pandas DataFrame}
...
\section{CLI、PowerShell、Bash、環境変数、終了コード}
...
\section{rclpy、rcl、rmw、DDS/RTPSの層}
...
\section{ROS graph、Node、topic、service、action、parameter}
...
\section{callback、timer、Executor、spin、Callback Group}
...
\section{launch Action、Node Action、実行可能名、remapping}
...
\section{rqt\_graph、ros2 CLI、rosbag2をいつ使うか}
...
\section{目的関数、制約、L-BFGS-B、SHGO、有限grid証明}
...
\section{Cross-Entropy Methodを式と実装で理解する}
...
\section{warm startは何を保存し、どう効くか}
...
\section{freshness、filter、guard、fallback、fail-safe}
...
\section{CSV/YAMLのdata contractと検証}
...
\section{単体試験、SILS、replay、観測可能性}
