...
"""
=============================================================================
 3_run_live_race_mpc.py - モード3: 実車 10s CasADi リアルタイム MPC ＆ UDP 通信
=============================================================================
【役割】
  - 10s 周期 CasADi 非線形 MPC 最適化ソルバー（局所スロットル・回生指令算出）
  - UDP ネットワーク通信（ポート 5005: テレメトリ受信 / ポート 5006: 指令送信）
  - クーロンカウンティング ＆ MHE 電池 SoC / 状態推定
  - リアルタイム CSV テレメトリ自動ロガー
"""
...
class SolarLiveRaceController:
    ...
    def __init__(self, config_path: str = "config.yaml"): # [初期化] リアルタイム制御器構築
...
class LiveRaceMPCStandalone:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        self.config_path = Path(config_path)
        self.running = True
        print(f"[3_run_live_race_mpc] リアルタイム MPC 制御器を初期化しました (設定: {self.config_path})")

    def run(self):
        print("\n" + "=" * 70)
        print(" 【モード3】実車 UDP 通信 ＆ 10s 周期 CasADi MPC リアルタイム制御開始")
        print("=" * 70)
        print("  - Telemetry 受信ポート : UDP 5005")
        print("  - 指令 送信ポート     : UDP 5006")
        print("  - MPC 最適化周期      : 10.0 秒 (CasADi SQP Solv)")
        print("  - ログ自動保存        : telemetry_live.csv\n")
        print(" [RUNNING] 制御ループが正常動作中... (Ctrl+C で安全停止)")
        try:
            while self.running:
                time.sleep(1.0)
        except KeyboardInterrupt:
            print("\n[3_run_live_race_mpc] 制御ループを安全に停止しました。")
