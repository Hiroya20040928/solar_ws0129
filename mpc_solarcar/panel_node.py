...
class PanelNode(Node):
    def __init__(self):
        ...
        self.sub_throttle = self.create_subscription(Float32, '/planner/throttle_cmd_pct', self.on_throttle, 10)
        self.sub_path = self.create_subscription(Path, '/planner/trajectory', self.on_path, 10)
        self.sub_gps = self.create_subscription(NavSatFix, '/sim/gps', self.on_gps, 10)
        self.sub_state = self.create_subscription(String, '/system/state', self.on_state, 10)
        self.sub_diag = self.create_subscription(String, '/system/diag', self.on_diag, 10)
        self.sub_idle = self.create_subscription(Float32, '/vehicle/idle_fuel_lph', self.on_idle, 10)
        self.sub_grade = self.create_subscription(Float32, '/vehicle/grade', self.on_grade, 10)
        self.sub_status = self.create_subscription(Float32MultiArray, '/planner/status', self.on_status, 10)
    ...
    def on_throttle(self, msg: Float32):
    ...
    def on_idle(self, msg: Float32):
        self.idle_fuel = float(msg.data)
    def on_grade(self, msg: Float32):
    ...
    def _status_text(self):
        ...
        return (f'State: {self.system_state} | {self.system_diag} | '
