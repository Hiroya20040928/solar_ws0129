...
class MPCNode(Node):
    ...
    def _mpc_solve_solar(self, data):
        ...
        if np.all(np.isfinite(res.x)):
            ...
            v_seq = res.x
            if not res.success:
                self.get_logger().warn('Upper MPC reached an early stop; using best finite iterate.')
        else:
    ...
    def _mpc_solve_solar_distance(self, t0_utc: datetime, s0_km: float, x0=None):
        ...
        if np.all(np.isfinite(res.x)):
            ...
            u_seq = res.x
            if not res.success:
                self.get_logger().warn('Upper distance-MPC reached an early stop; using best finite iterate.')
        else:
            self.last_upper_solve_ok = False
            self.get_logger().warn('Upper distance-MPC solve failed; reusing warm-start control grid.')
            u_seq = x0
    ...
    def _lower_mpc_solve(self, v0_ms: float, s0_km: float, z0: float, Tb0: float, env: dict, v_ref_seq):
        ...
        if np.all(np.isfinite(res.x)):
            ...
            u_seq = res.x
            if not res.success:
                self.get_logger().warn('Lower MPC reached an early stop; using best finite iterate.')
        else:
            self.last_lower_solve_ok = False
            self.get_logger().warn('Lower MPC solve failed; holding previous throttle seed.')
            u_seq = x0
